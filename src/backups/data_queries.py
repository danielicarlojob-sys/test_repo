...
def execute_query_from_file(
        sql_file: str, conn: pyodbc.Connection) -> Union[pd.DataFrame, str, None]:
    """
...
def fetch_all_flight_phase_data(path: str) -> pd.DataFrame:
    ...
    for file in os.listdir(sql_files_path):
        ...
        if file.endswith('CRZ.sql'):
            flight_phase = 'Cruise'
            data, timestamp_str_initial = execute_query_from_file(file, conn)
            if not data.empty:
                # extract latest timestamp from the query just run
                latest_ts = data['reportdatetime'].max()
                # append latest_ts to timestamps list
                timestamps.append(latest_ts)
                # Merge historical data from previous run with query output
                data = df_merger(data, flight_phase)
                file_name = 'data_cruise.csv'
                file_path = os.path.join(data_dir, file_name)
                data = filter_latest_n_pts_per_esn(
                    data, file_path, flight_phase, timestamp_str_initial)
