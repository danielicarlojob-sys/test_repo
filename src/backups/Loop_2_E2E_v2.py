...
def Loop_2_E2E(
    ...
    df_old = df_temp[df_temp['NEW_FLAG']!=1]
    ...
    if frames:
        ...
        # if both are empty, create an empty dfwith expected columns
        df_conc = pd.DataFrame(columns=df_temp.columns)
    ...
    if not df_conc.empty:
        df_conc = df_conc.sort_values(
            by='reportdatetime',
            ascending=False).drop_duplicates(keep='last')
