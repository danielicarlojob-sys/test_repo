...
def merge_flight_phases(data_dict: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    """Merge take-off, climb and cruise dataframes into one, matching each phase
    in chronological order and enforcing a 1-hour window from take-off to cruise.

    This function does the following:
    1. Ensures `reportdatetime` is a datetime type and sorts each DataFrame.
    2. Uses an “as-of” merge to pair each take-off row with the earliest climb
       after it, then each take-off+climb pair with the earliest cruise after the climb.
    3. Enforces that the total time from take-off to cruise does not exceed one hour.
       If any phase falls outside this window or has no match, its columns are set to NaT/NaN.
    4. Returns a DataFrame with one row per take-off, plus columns for the
       corresponding climb and cruise `reportdatetime` and `row_sum`.

    Args:
        df_takeoff (pd.DataFrame): take-off data, with columns
            ['ESN','operator','ACID','ENGPOS','DSCID','reportdatetime','row_sum'].
            DSCID must be 53 for take-off.
        df_climb (pd.DataFrame): climb data, same columns, DSCID == 54.
        df_cruise (pd.DataFrame): cruise data, same columns, DSCID == 52.

    Returns:
        pd.DataFrame: one row per take-off, with columns
            ...
            Unmatched or out-of-window phases will have NaT (for datetimes)
            or NaN (for row_sum).
    """
    # Unpack
    df_takeoff = data_dict['take-off']
    df_climb   = data_dict['climb']
    df_cruise  = data_dict['cruise']

    group_keys = ['ESN', 'operator', 'ACID', 'ENGPOS']

    # 1) Ensure sorting really happens on each DataFrame
    for phase, df in zip(
        ['takeoff', 'climb', 'cruise'],
        [df_takeoff, df_climb, df_cruise]
    ):
        df = df.copy()
        df['reportdatetime'] = pd.to_datetime(df['reportdatetime'])
        df.sort_values(group_keys + ['reportdatetime'], inplace=True)
        # reassign back
        if phase == 'takeoff':
            df_takeoff = df
        elif phase == 'climb':
    ...
    # 2) Merge take-off → climb
    takeoff_climb = pd.merge_asof(
        left=df_takeoff,
        right=df_climb,
        on='reportdatetime',
        by=group_keys,
        direction='forward',
        suffixes=('_take-off', '_climb')
    )

    # 3) Merge (take-off+climb) → cruise
    takeoff_climb_cruise = pd.merge_asof(
        left=takeoff_climb.sort_values(group_keys + ['reportdatetime_climb']),
        right=df_cruise,
        left_on='reportdatetime_climb',
        right_on='reportdatetime',
        by=group_keys,
        direction='forward'
    )

    # … rest of your renaming + window‐filter logic unchanged …
...
def Loop_9_combine_DSC(
    ...
    for fp in data_dict.keys():
        data_dict_shrinked[fp] = data_dict[fp][cols]
    try:
        df_merged = merge_flight_phases(data_dict_shrinked) 
