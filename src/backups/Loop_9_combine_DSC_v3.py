...
def Loop_9_grouping(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in grouped:
        ...
        take_off_group_sorted = group_sorted[group_sorted['DSCID']==53].copy()
        climb_group_sorted = group_sorted[group_sorted['DSCID']==54].copy()
        cruise_group_sorted = group_sorted[group_sorted['DSCID']==52].copy()
        for idx, rdt in enumerate(take_off_group_sorted['reportdatetime']):
            climb_datapoint = climb_group_sorted[climb_group_sorted['reportdatetime'] > rdt]
